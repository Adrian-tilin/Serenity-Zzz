...
$(function () {
  ...
  // Update perfil link based on login status
  const perfilLink = $('#quick-panel a[href="#perfil"]');
  if (perfilLink.length) {
    const token = localStorage.getItem('token');
    const user = JSON.parse(localStorage.getItem('user') || 'null');
    if (token && user) {
      perfilLink.attr('href', 'Perfil/Perfil-edit.html');
    } else {
      perfilLink.attr('href', 'login-register.html');
    }
  }
